...
export default function PurchasesPage() {
  ...
  const confirmDelete = async () => {
    ...
    try { await deletePurchase.mutateAsync(deleteTarget.id); toast('success', 'Order deleted'); }
          ...
          <div className="space-y-4">
            ...
            <table className="w-full text-sm border rounded-lg overflow-hidden">
              <thead className="bg-gray-50 dark:bg-gray-700">
                <tr>{['Product', 'Qty', 'Unit Cost', 'Total'].map(h => <th key={h} className="px-3 py-2 text-left text-xs font-semibold text-gray-500">{h}</th>)}</tr>
              </thead>
